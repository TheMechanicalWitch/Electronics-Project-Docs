#!/usr/bin/env python3
"""
3D arm tracking on an OAK-D Lite: joint coordinates (X, Y, Z in meters)
and the elbow angle (between upper arm and forearm), for both arms.

How it works
------------
1. MediaPipe Pose (runs on your computer) finds shoulder / elbow / wrist
   in the RGB image.
2. The OAK-D Lite's stereo depth map — aligned to the RGB camera — gives
   the distance (Z) at each joint's pixel.
3. The camera's factory calibration (intrinsics) converts pixel + depth
   into metric X, Y, Z in the camera's coordinate frame.
4. The elbow angle is computed between the 3D vectors elbow->shoulder and
   elbow->wrist, so it is the true anatomical angle, independent of the
   direction you are facing.

Coordinate frame (origin = RGB camera lens):
    X: right (from the camera's point of view), meters
    Y: down, meters  (negate it if you prefer Y up)
    Z: forward, away from the camera, meters

Install (note: plain `pip install depthai` gives the new v3 API,
this script uses the stable v2 API, hence the pin):

    pip install "depthai==2.32.0.0" mediapipe opencv-python numpy

Run:
    python arm_angle_3d.py       (press q or Esc to quit)
...
def angle_at(b: np.ndarray, a: np.ndarray, c: np.ndarray):
        ...
        while True:
            ...
            if result.pose_landmarks:
                ...
                for side, idx in ARMS.items():
                    ...
                    shoulder, elbow, wrist = pts3d
                    ang = angle_at(elbow, shoulder, wrist)
                    ...
                    if ang is not None:
                        cv2.putText(frame, f"{ang:5.1f} deg",
                                    (pts2d[1][0] + 10, pts2d[1][1] - 10),
                    ...
                    for name, P in zip(("shoulder", "elbow", "wrist"), pts3d):
                        ...
                        report.append(line)
                    if ang is not None:
                        line = f"{side} elbow angle {ang:6.1f} deg"
                        cv2.putText(frame, line, (8, y_txt),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                        y_txt += 22
                        report.append(line)

            if report and time.time() - last_print > 0.5:
                print("\n".join(report), end="\n\n")
